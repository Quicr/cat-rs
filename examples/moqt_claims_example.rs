...
use cat_impl::{decode_token, encode_token};
...
fn demonstrate_basic_moqt_token() {
    ...
    let test_cases = vec![
        (
            ...
            "cdn.example.com".as_bytes(),
            "/sports/football".as_bytes(),
            true,
        ),
        (
            MoqtAction::Publish,
            "cdn.example.com".as_bytes(),
            "/sports/basketball/live".as_bytes(),
            true,
        ),
        (
            MoqtAction::Fetch,
            "cdn.example.com".as_bytes(),
            "/sports/tennis".as_bytes(),
            false,
        ), // Not allowed
        (
            MoqtAction::PublishNamespace,
            "other.example.com".as_bytes(),
            "/sports/soccer".as_bytes(),
            false,
        ), // Wrong namespace
        (
            MoqtAction::PublishNamespace,
            "cdn.example.com".as_bytes(),
            "/news/breaking".as_bytes(),
            false,
        ), // Wrong track prefix
    ];
...
fn demonstrate_spec_examples() {
    ...
    assert!(exact_token.allows_moqt_action(
        &MoqtAction::PublishNamespace,
    ...
    assert!(prefix_token.allows_moqt_action(
        &MoqtAction::PublishNamespace,
        "example.com".as_bytes(),
        "/bob".as_bytes()
    ));
...
fn demonstrate_multiple_scopes() {
    ...
    let test_cases = vec![
        ...
        (
            MoqtAction::PublishNamespace,
            "live.example.com".as_bytes(),
            "/stream/sports".as_bytes(),
            true,
        ),
        (
            MoqtAction::Publish,
            "live.example.com".as_bytes(),
            "/stream/news/breaking".as_bytes(),
            true,
        ),
        (
            MoqtAction::Fetch,
            "live.example.com".as_bytes(),
            "/stream/music".as_bytes(),
            false,
        ), // Wrong action
        // VOD tests
        (
            MoqtAction::Fetch,
            "vod.example.com".as_bytes(),
            "movie.mp4".as_bytes(),
            true,
        ),
        (
            ...
            "vod.example.com".as_bytes(),
            "series/episode1.mp4".as_bytes(),
            true,
        ),
        (
            MoqtAction::Publish,
            "vod.example.com".as_bytes(),
            "content.mp4".as_bytes(),
            false,
        ), // Wrong action
        (
            MoqtAction::Fetch,
        ...
        (
            MoqtAction::TrackStatus,
            "admin.example.com".as_bytes(),
            "admin-dashboard".as_bytes(),
            true,
        ),
        (
            MoqtAction::SubscribeNamespace,
            "admin.internal.net".as_bytes(),
            "user-admin".as_bytes(),
            true,
        ),
        (
            MoqtAction::PublishNamespace,
            "public.example.com".as_bytes(),
            "admin-panel".as_bytes(),
            false,
...
fn demonstrate_token_encoding_decoding() {
    ...
    println!(
        ...
        decoded_token.moqt.moqt_reval.unwrap()
    );
...
fn demonstrate_binary_matching_patterns() {
    ...
    println!(
        "  ✗ 'live-stream-hd' -> {}",
        exact_match.matches("live-stream-hd".as_bytes())
    );
    println!(
        "  ✗ 'my-live-stream' -> {}",
        exact_match.matches("my-live-stream".as_bytes())
    );
    ...
    println!(
        "  ✓ '/api/v1/users/123' -> {}",
        prefix_match.matches("/api/v1/users/123".as_bytes())
    );
    println!(
        "  ✗ '/api/v2/streams' -> {}",
        prefix_match.matches("/api/v2/streams".as_bytes())
    );
    println!(
        "  ✗ '/v1/api/streams' -> {}",
        prefix_match.matches("/v1/api/streams".as_bytes())
    );
    ...
    println!(
        "  ✓ '/path/to/stream.webm' -> {}",
        suffix_match.matches("/path/to/stream.webm".as_bytes())
    );
    println!(
        "  ✗ 'video.mp4' -> {}",
        suffix_match.matches("video.mp4".as_bytes())
    );
    println!(
        "  ✗ 'video.webm.tmp' -> {}",
        suffix_match.matches("video.webm.tmp".as_bytes())
    );
    ...
    println!(
        "  ✓ '/complex/path/123' -> {}",
        empty_match.matches("/complex/path/123".as_bytes())
    );
}
...
fn demonstrate_revalidation_claim() {
    ...
    println!(
        "  Live streaming: {} seconds",
        stream_token.moqt.moqt_reval.unwrap_or(0.0)
    );
    println!(
        "  Read-only access: {} seconds",
        readonly_token.moqt.moqt_reval.unwrap_or(0.0)
    );
    println!(
        "  Persistent access: {} (none)",
        if persistent_token.moqt.moqt_reval.is_some() {
            "some"
        } else {
            "none"
        }

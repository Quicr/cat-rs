...
use cat_impl::claims::{CatToken, CompositeClaim, CompositeOperator, composite_utils};
...
fn demonstrate_or_composite() {
    ...
    let region_composite =
        composite_utils::create_or_from_tokens(vec![us_token, eu_token, apac_token]);
...
fn demonstrate_and_composite() {
    ...
    let service_composite =
        composite_utils::create_and_from_tokens(vec![auth_token, data_token, audit_token]);
...
fn demonstrate_nor_composite() {
    ...
    let restricted_composite =
        composite_utils::create_nor_from_tokens(vec![restricted_token1, restricted_token2]);
...
fn demonstrate_nested_composite() {
    ...
    let time_composite =
        composite_utils::create_or_from_tokens(vec![business_token, emergency_token]);
...
fn demonstrate_composite_claims_container() {
    ...
    println!(
        "Token has composites: {}",
        complex_token.composite.has_composites()
    );
...
fn demonstrate_composite_creation_utilities() {
    ...
    println!(
        "Created OR composite with {} claims",
        or_composite.claims.len()
    );
    println!(
        "Created AND composite with {} claims",
        and_composite.claims.len()
    );
    println!(
        "Created NOR composite with {} claims",
        nor_composite.claims.len()
    );
...
fn demonstrate_performance_considerations() {
    ...
    for level in 1..=3 {
        ...
        new_composite.add_token(create_token_for_service(
            &format!("level-{}", level),
            "nested",
        ));

...
fn test_uri_matcher_comprehensive() {
    ...
    matcher.add_pattern(UriPattern::Exact("https://api.example.com/v1".to_string())).unwrap();
    matcher.add_pattern(UriPattern::Prefix("https://secure.".to_string())).unwrap();
    matcher.add_pattern(UriPattern::Suffix("/api/data".to_string())).unwrap();
    matcher.add_pattern(UriPattern::Regex(r"^https://.*\.example\.com/(users|login|[^v].*)$".to_string())).unwrap();
...
fn test_uri_matcher_get_matching_patterns() {
    ...
    matcher.add_pattern(UriPattern::Prefix("https://".to_string())).unwrap();
    matcher.add_pattern(UriPattern::Suffix(".com".to_string())).unwrap();
    matcher.add_pattern(UriPattern::Exact("https://example.com".to_string())).unwrap();

...
fn test_token_builder_network_methods() {
    ...
    assert!(nips.iter().any(|nip| matches!(nip, NetworkIdentifier::IpAddress(ip) if ip == "203.0.113.1")));
    assert!(nips.iter().any(|nip| matches!(nip, NetworkIdentifier::IpRange(range) if range == "198.51.100.0/24")));
    assert!(nips.iter().any(|nip| matches!(nip, NetworkIdentifier::Asn(asn) if *asn == 64496)));
...
fn test_incremental_network_identifier_building() {
    ...
    let nips = token.cat.catnip.unwrap();
    assert!(nips.iter().any(|nip| matches!(nip, NetworkIdentifier::IpAddress(ip) if ip == "10.1.1.1")));
    assert!(nips.iter().any(|nip| matches!(nip, NetworkIdentifier::Asn(asn) if *asn == 65001)));
    assert!(nips.iter().any(|nip| matches!(nip, NetworkIdentifier::IpRange(range) if range == "192.168.0.0/16")));
...
fn test_network_identifier_cwt_encoding_decoding() {
    ...
    assert!(decoded_nips.iter().any(|nip| matches!(nip, NetworkIdentifier::IpAddress(ip) if ip == "203.0.113.42")));
    assert!(decoded_nips.iter().any(|nip| matches!(nip, NetworkIdentifier::IpRange(range) if range == "198.51.100.0/24")));
    assert!(decoded_nips.iter().any(|nip| matches!(nip, NetworkIdentifier::Asn(asn) if *asn == 64512)));
...
fn test_mixed_network_identifiers_comprehensive() {
    ...
    assert_eq!(token.core.iss, Some("https://network.example.com".to_string()));
